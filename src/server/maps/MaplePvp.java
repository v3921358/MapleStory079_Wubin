...
public class MaplePvp {
    ...
    private static void monsterBomb(MapleCharacter player, MapleCharacter attackedPlayers, MapleMap map, AttackInfo attack) {
        ...
        int magicattack = (player.getDex() + player.getInt() + player.getLuk() + player.getStr()) / 300; //4項屬性值最後除以3=magicattack K攻擊力+magicattack值
        pvpDamage += magicattack;

        int magicat = (player.getStat().getTotalMagic() + player.getStat().getTotalWatk()) / 100; //魔法+物理攻擊除以100=magicat值 PK攻擊力+magicat值
        pvpDamage += magicat;
        ...
        attackedPlayers.getClient().sendPacket(MaplePacketCreator.getErrorNotice(player.getName() + " 打了 " + attackedDamage + " 點的傷害!"));
