            ...
            if (MapleItemInformationProvider.getInstance().itemExists(c.right.getItemId()) && c.right.getChance() > 0) {
                switch (type) {
                    ...
                    case "2":
                        ...
                        break;
                    case "GM":
                        sb.append("#L" + c.right.getItemId() + "##v" + c.right.getItemId() + "##z" + c.right.getItemId() + "# 几率:" + c.right.getChance() + "# 数量:" + c.right.getQuantity() + "(点击更改)\r\n");
                        break;
                    default:
                        sb.append("显示错误!");
                        break;
...
//            System.out.println(c.right.getItemId());            
            if (MapleItemInformationProvider.getInstance().itemExists(c.right.getItemId())) {
                sb.append("#L" + items.size() + "##v" + c.right.getItemId() + "##z" + c.right.getItemId() + "# 几率:" + c.right.getChance() + "# 数量:" + c.right.getQuantity() + "(点击更改)\r\n");
            }
        }
        return sb.toString();
    }
...
//                System.out.println(chance+" "+getId()+" "+itemid);
        ...
        World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密語系統] " + chr.getName() + "  更改物品:" + MapleItemInformationProvider.getInstance().getName(itemid) + " 機率更改為" + chance));
...
//                System.out.println(chance+" "+getId()+" "+itemid);
        ...
        World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密語系統] " + chr.getName() + "  更改物品:" + MapleItemInformationProvider.getInstance().getName(itemid) + " 數量更改為" + quantity));
...
//                System.out.println(chance+" "+getId()+" "+itemid);
        ...
        World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密語系統] " + chr.getName() + "  新增轉蛋物品:" + MapleItemInformationProvider.getInstance().getName(itemid) + " 機率設定為" + chance + " 是否上綠廣:" + (msg ? "是" : "否") + " 數量設定為" + quantity));
