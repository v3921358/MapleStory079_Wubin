...
public class BeanGame {
 ...
 /*public static int[] 豆豆装备 = {1002695, 1002609, 1002665, 1002985, 1002986, 1002761, 1002760, 1002583, 1002543, 1002448, 1052137, 1092051, 1702232, 1702138};
    ...
    public static int[] 消耗品 = {2022140, 2022141, 2022139, 2022137, 1902035, 1902037};*/
            ...
            case 5://应该是普通的怪物中奖 //移动以后三排一样的 这里应该是处理 打完以后出现的 看看是不是判断一排的代码
                ...
                if ((第一排 == 0 && 第三排 == 0 && 第二排 == 0)
                    ...
                    if (Randomizer.nextInt(100) < 50) {
                        ...
                        itemId = Integer.parseInt(消耗品[Randomizer.nextInt(消耗品.length)]);

                        /*if (Randomizer.nextInt(100) < 30) {
                            itemId = Integer.parseInt(豆豆装备[Randomizer.nextInt(豆豆装备.length)]);
                            if (Randomizer.nextInt(100) < 10) {
                                itemId = Integer.parseInt(豆豆坐骑[Randomizer.nextInt(豆豆坐骑.length)]);
                            }
                        }*/
                    }
                    ...
                    if (c.getPlayer().getInventory(GameConstants.getInventoryType(itemId)).getNextFreeSlot() > -1 && itemId != 0) {
                        ...
                        MapleInventoryManipulator.addById(c, itemId, (short) count);
                        chr.setBossLog("欢乐豆豆");
                    }
                    ...
                    if (chr.getMapId() == 809030000) {
                        String notea = "恭喜你小鋼珠成功中獎！當前中獲得：" + 奖励豆豆 + "小鋼珠積分！" + (itemId <= 0 ? "" : MapleItemInformationProvider.getInstance().getName(itemId));
        ...
        if (type == 0x0B || type == 6) {
            for (int i = 0; i < 5; i++) {
                beansInfo.add(new MapleBeans(rand(3000, 5000)/*rand(4350, 4450)*//*rand(1000, 5000)*/, getBeanType(), chr.getBeansNum() + i));
            }
